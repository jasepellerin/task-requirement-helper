...
- **Detail card** (click a tile):
  ...
  - Quest cards show difficulty and length as pills, Gold when the wiki lists required coins, Unlocks lines with the Slayer Master icon, Slayer skill icon, Transportation icon, teleport spell icon, and/or teleport-item sprite when the quest unlocks a master, Slayer monsters, a transport network, a teleport spell, or a teleport item, and an Items section for required items (wiki how-to-get notes are dropped).
