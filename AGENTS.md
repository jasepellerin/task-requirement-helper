...
- **Detail card** (click a tile):
  ...
  - Quest cards show difficulty and length as pills, Gold when the wiki lists required coins, Unlocks lines with the Slayer Master icon, Slayer skill icon, Transportation icon, spellbook icon, teleport spell icon, teleport-item sprite, and/or minigame map icon when the quest unlocks a master, Slayer monsters, a transport network, a magic spellbook, a teleport spell, a teleport item, or a minigame, and an Items section for required items (wiki how-to-get notes are dropped).
